...
def parse_args():
    ...
    parser.add_argument("--use_pos_emb", dest="use_pos_emb", action="store_true",
                        help="Use learned positional embeddings (default).")
    parser.add_argument("--no_pos_emb", dest="use_pos_emb", action="store_false",
                        help="Use sinusoidal positional encodings instead of learned embeddings.")
    parser.set_defaults(monosemantic_enabled=False)  # disable by default
    parser.set_defaults(use_pos_emb=True)  # learned positional embeddings by default
    ...
    parser.add_argument("--inference_ckpt", type=str, default=None,
                        help="If provided, run inference with this checkpoint and exit (skips training).")

    parser.add_argument("--epochs", type=int, default=3, help="Number of training epochs (was missing).")
    ...
    parser.add_argument("--norm", type=str, choices=["pre", "post"], default="pre", help="Normalization style for Transformer blocks.")
    parser.add_argument("--use_swiglu", action="store_true", help="Use SwiGLU feedforward in Transformer.")
    parser.add_argument("--kgram_use_embedding", action="store_true", help="Use embeddings for k-gram model (efficient path).")
    parser.add_argument("--model_type", type=str, default="all", choices=["all","kgram","lstm","transformer"], help="Which model(s) to train.")
    ...
    parser.add_argument("--ema_decay", type=float, default=1.0, help="EMA decay (set <1.0 to enable EMA).")
...
class FeedForward(nn.Module):
    """A simple 2-layer MLP, with optional SwiGLU."""
    def __init__(self, d_model, d_ff, use_swiglu: bool = False):
        super().__init__()
        self.use_swiglu = use_swiglu

        if use_swiglu:
            # Official SwiGLU: a single projection to 2*d_ff, then split
...
class TransformerBlock(nn.Module):
    ...
    """
    def __init__(self, d_model, n_heads, d_ff, use_flash: bool = False, norm_type: str = "pre", use_swiglu: bool = False):
        super().__init__()
        ...
        self.ff = FeedForward(d_model, d_ff, use_swiglu=use_swiglu)
...
class TransformerModel(nn.Module):
    ...
    """
    def __init__(self, vocab_size=50257, d_model=256, n_heads=4, n_blocks=4, block_size=256, use_flash: bool = False, weight_tying: bool = False, norm_type: str = "pre", use_swiglu: bool = False, use_pos_emb: bool = True):
        super().__init__()
        ...
        if self.use_pos_emb:
            # learned positional embeddings (existing behavior)
            self.pos_embedding = nn.Embedding(block_size, d_model)
        else:
            # sinusoidal positional encoding buffer (non-learned)
            ...
            self.register_buffer('pos_encoding', pe, persistent=True)

        d_ff = d_model * 4
        if use_swiglu:
            # As per SwiGLU paper, hidden dim is often 2/3 of the FFN dim
            d_ff = int(d_ff * 2 / 3)
        self.blocks = nn.ModuleList([
            TransformerBlock(d_model, n_heads, d_ff, use_flash=use_flash, norm_type=norm_type, use_swiglu=use_swiglu) for _ in range(n_blocks)
        ])
    ...
    def forward(self, tokens_seq):
        ...
        if self.use_pos_emb:
            ...
            # use registered sinusoidal encoding; cast/move to match current tensor dtype/device
            pos_emb = self.pos_encoding[:seq_len].unsqueeze(1).to(device=tok_emb.device, dtype=tok_emb.dtype)
...
def monosemantic_analysis_for_token(token_id, model, enc, device="cpu", top_n=5):
    """
    Finds tokens with the most similar embeddings to a given token_id.
    This requires the model to have a `token_embedding` or `embedding` attribute.
    """
    ...
    with torch.no_grad():
        ...
        embedding_matrix = embedding_layer.weight.detach() # (V, D)
        
        # Get the embedding for the target token
        target_embedding = embedding_matrix[token_id] # (D,)
        
        # Compute cosine similarity
        similarities = F.cosine_similarity(target_embedding.unsqueeze(0), embedding_matrix, dim=1)
        
        # Get top N, excluding the token itself
        top_k_sims, top_k_indices = torch.topk(similarities, k=top_n + 1)
...
def inference_results(model, enc, device, args):
    """
    Run inference on a loaded model, generate text with various sampling
    strategies, and print results including monosemantic analysis.
    """
    print("\n=== Running Inference ===")
    model.eval()
    ...
    for kind, p in sampling_strategies:
        header = f"{kind} p={p}" if p is not None else f"{kind}"
        print(f"\n--- Generating with {header} ---")
...
def main():
    ...
    # Inference-only mode
    if args.inference_ckpt:
        print(f"Inference mode enabled. Loading checkpoint: {args.inference_ckpt}")
        ...
        # Instantiate model (assuming Transformer)
        # User must provide matching architecture args
        print("Loading Transformer model with specified architecture...")
        ...
        # Load checkpoint
        try:
            checkpoint = torch.load(args.inference_ckpt, map_location=device)
            # Handle both regular and EMA state dicts
            if 'ema_state_dict' in checkpoint and checkpoint['ema_state_dict']:
                print("Loading EMA model state from checkpoint.")
                # The EMA state might be on CPU, ensure it's moved to device
                ema_sd = {k: v.to(device) for k, v in checkpoint['ema_state_dict'].items()}
                model.load_state_dict(ema_sd)
            elif 'model_state_dict' in checkpoint:
                print("Loading standard model state from checkpoint.")
                model.load_state_dict(checkpoint['model_state_dict'])
            else:
                # Fallback for raw state dict checkpoints
                print("Checkpoint does not contain 'model_state_dict' or 'ema_state_dict', attempting to load root object.")
        ...
        # Run inference
        inference_results(model, enc, device, args)
    ...
    for model_name, model in models.items():
        if model_name == "transformer":
            ...
            # Positional embeddings may be learned (nn.Embedding) or a fixed sinusoidal buffer (pos_encoding)
            if hasattr(model, "pos_embedding") and getattr(model, "use_pos_emb", False):
                pos_emb_params = count_params(model.pos_embedding)
                pos_emb_desc = f"{pos_emb_params:,} parameters (learned)"
            else:
                # sinusoidal positional encodings are registered as a buffer and are non-trainable
                pos_emb_params = 0
                pos_emb_desc = "0 parameters (sinusoidal, non-learnable)"

            print(f"  Token Embedding: {tok_emb_params:,} parameters")
            print(f"  Positional Embedding: {pos_emb_desc}")
